...
from rubis_msgs.msg import PoseTwistStamped
...
def write_position_info():
    ...
    with open(center_offset_file, "w") as f:
        ...
        while not rospy.is_shutdown():
            ...
            instance=rubis_current_pose_twist_msg.instance

            gnss_pose_x = round(rubis_current_pose_twist_msg.pose.pose.position.x, 3)
            gnss_pose_y = round(rubis_current_pose_twist_msg.pose.pose.position.y, 3)
            gnss_ori_x = rubis_current_pose_twist_msg.pose.pose.orientation.x
            gnss_ori_y = rubis_current_pose_twist_msg.pose.pose.orientation.y
            gnss_ori_z = rubis_current_pose_twist_msg.pose.pose.orientation.z
            gnss_ori_w = rubis_current_pose_twist_msg.pose.pose.orientation.w                                                         
            ...
            current_pose_x = round(rubis_current_pose_twist_msg.pose.pose.position.x, 3)
            current_pose_y = round(rubis_current_pose_twist_msg.pose.pose.position.y, 3)
            ndt_ori_x = rubis_current_pose_twist_msg.pose.pose.orientation.x
            ndt_ori_y = rubis_current_pose_twist_msg.pose.pose.orientation.y
            ndt_ori_z = rubis_current_pose_twist_msg.pose.pose.orientation.z
            ndt_ori_w = rubis_current_pose_twist_msg.pose.pose.orientation.w                 
            current_velocity = rubis_current_pose_twist_msg.twist.twist.linear.x
            if current_velocity > 20: continue

            center_offset_wr.writerow([time.clock_gettime(time.CLOCK_MONOTONIC), state_text, str(min_dis), 'None', instance, gnss_pose_x, gnss_pose_y, 0.0, current_pose_x, current_pose_y, current_velocity])
    f.close()
