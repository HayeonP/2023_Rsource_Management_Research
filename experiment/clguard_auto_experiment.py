...
def seqwr_workload(core, budget):
    os.system(f"ssh root@192.168.0.11 \"taskset -c {core} /var/lib/lxc/linux1/rootfs/home/root/mg/tools/source/sequential_write -b {budget_to_target_convert_table[budget]} -s 64\" > /dev/null")
    time.sleep(1)
    return
...
def is_clguard_installed():
    ...
    for kernel_module in kernel_module_list:
        if 'memguard' in kernel_module:
            return True
    return False

def insmod_clguard():
    os.system(f'ssh {ssh_address} "cd {exynos_clguard_dir} && sh run.sh"')
    time.sleep(1)

def rmmod_clguard():
    os.system(f'ssh {ssh_address} "cd {exynos_clguard_dir} && rmmod memguard.ko"')
...
def adas_experiment():
    ...
    insmod_clguard()
    set_clguard_limit('adas', '4-7', adas_budget)

    # ADAS (w/ Clguard) only
    label = f'{experiment_title}_b{adas_budget}_adas'
    ...
    label = f'{experiment_title}_b{adas_budget}_adas_seqwr{seqwr_budget}'
    update_adas_config(label)
    ...
    set_clguard_limit('seqwr', '1-3', seqwr_budget)

    # ADAS (w/ Clguard) + Seqwr (w/ Clguard)
    label = f'{experiment_title}_b{adas_budget}_adas_b{seqwr_budget}_seqwr'
    update_adas_config(label)
